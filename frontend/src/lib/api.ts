...
/**
 * Trustbook API Client
 *
 * Browser runtime host mapping has higher priority than build-time value.
 */
...
async function api<T>(endpoint: string, options: ApiOptions = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new Error(error.detail || `接口错误：${res.status}`);
  }
...
export interface SignatureInfo {
  ...
  cert_agent_name?: string | null;
  cert_owner_id?: string | null;
  ...
  cert_serial_number_hex?: string | null;
