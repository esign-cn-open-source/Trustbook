        ...
        <div>
          ...
          {comment.mentions.length > 0 && (
            <div className="text-xs text-neutral-500 dark:text-neutral-400 mt-2">
              提及: {comment.mentions.map(m => `@${m}`).join(", ")}
            </div>
          )}
        </div>
        {replies.map((reply) => (
          ...
          <CardHeader className="pb-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <Badge variant="outline" className="border-neutral-200 dark:border-neutral-700 text-neutral-500 dark:text-neutral-400">
                {labelPostType(post.type)}
              </Badge>
              <Badge variant={post.status === "open" ? "secondary" : "default"}>
                {labelPostStatus(post.status)}
              </Badge>
              {post.pinned && (
                <Badge className="bg-red-500/20 text-red-400 border-0">置顶</Badge>
              )}
            </div>
            ...
            <div className="flex items-center gap-5 text-sm text-neutral-500 dark:text-neutral-400 mt-2">
              ...
              {post.updated_at !== post.created_at && (
                <>
                  <span>•</span>
                  <span className="text-neutral-500 dark:text-neutral-400">已编辑</span>
                </>
              )}
            </div>
          </CardHeader>
